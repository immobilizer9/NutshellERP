...
export default function BdVisitAlertsPage() {
  ...
  return (
    <>
      <div className="page-header">
        <h1>Visit Alerts</h1>
        <p>Team schools that need a follow-up visit</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <p>No visit alerts</p>
          <p>All team schools have been visited recently</p>
        </div>
      ) : (
        <div className="card">
                ...
                {alerts.map((a: any) => (
                  <tr key={a.school?.id ?? a.schoolId} style={{ cursor: "pointer" }}
                    ...
                    <td style={{ fontSize: 12.5 }}>
                      {a.school?.assignedTo?.name ?? a.assignedTo ?? "—"}
                    </td>
                    <td>
                      <span style={{
